...
def build_llm_prompt_for_fund_setup(
    ...
    if search_intent:
        task_instruction = (
            ...
            f"Respond in JSON format with two keys: 'answer' and 'reasoning_steps'. "
            f"'answer' should be a clear sentence. 'reasoning_steps' should be a list of short steps showing how the answer was derived."
        )
    else:
        task_instruction = (
...
def evaluate_with_gemini(prompt: str, cache_path: str) -> dict:
    url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent"
    headers = {
        "Content-Type": "application/json",
        "X-goog-api-key": "AIzaSyATeJGWBSbWIC1TQs8F-GhwdWAkz2LoSHM"
    }
    ...
    if response.status_code == 200:
        try:
            ...
            return {"answer": f"[JSON parsing error: {str(e)}]", "reasoning_steps": []}
    else:
        print(f" Gemini API error {response.status_code}: {response.text}")
        return {"answer": f"[API Error {response.status_code}]", "reasoning_steps": []}
...
def parse_gemini_response(parsed: dict) -> Dict[str, Any]:
